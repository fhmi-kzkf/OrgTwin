...
app.post('/api/simulate', async (req: Request, res: Response) => {
  try {
    ...
    if (!objective || !context) return res.status(400).json({ error: 'Objective and context are required.' });

    const payloadSize = JSON.stringify(artifacts).length;
    if (payloadSize > 40_000_000) return res.status(413).json({ error: 'Artifacts payload too large. Max 40MB.' });
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'image' || artifact.type === 'pdf') {
        parts.push({ inlineData: { mimeType: artifact.mimeType, data: (artifact.data || '').split(',')[1] || artifact.data } });
      } else {
        parts.push({ text: `[DOCUMENT: ${sanitize(artifact.name, 200)} | FORMAT: ${(artifact.type || '').toUpperCase()}]\n${artifact.data}` });
      }
    }
                ...
                nodes: {
                  type: Type.ARRAY, items: {
                    type: Type.OBJECT, properties: {
                      ...
                      impact_score: { type: Type.INTEGER }, status: { type: Type.STRING, enum: ['risk', 'stable'] },
                ...
                links: {
                  type: Type.ARRAY, items: {
                    type: Type.OBJECT, properties: {
                      ...
                      relationship: { type: Type.STRING }, strength: { type: Type.NUMBER },
    ...
    if (result.allocation) result.allocation = normalizeAllocations(result.allocation);
    res.json(result);
  } catch (err: any) {
